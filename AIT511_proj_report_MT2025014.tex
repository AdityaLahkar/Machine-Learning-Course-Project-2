...
\begin{enumerate}
    \item \textbf{Logistic Regression}: Tuned \texttt{C} and \texttt{solver}.
    \item \textbf{SVM}: Tuned \texttt{C}, \texttt{gamma}, and \texttt{kernel} (RBF vs Linear). *Note: Subsampling (5000 samples) was used for tuning to optimize runtime.*
    \item \textbf{Neural Network}: Tuned \texttt{hidden\_layer\_sizes} and \texttt{learning\_rate}.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Logistic Regression}: Multinomial solver (\texttt{lbfgs}).
    \item \textbf{SVM}: RBF Kernel, effectively capped sample size.
    \item \textbf{Neural Network}: MLP with sizes like (100, 50), ReLu activation.
\end{enumerate}
